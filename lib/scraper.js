...
function parsePrice(text) {
  if (!text) return NaN;
  // Extract only the numeric block (e.g. "48,999.00" from "Rs. 48,999.00")
...
export async function scrapePrice(url, selector) {
  ...
  if (!response.ok) {
    if (response.status === 403) {
      throw new Error(`Failed to fetch page: HTTP 403 for ${url}. The website might be blocking automated access (Bot Protection).`);
    }
